...
关于新 C/C++ 标准的一些解释：
------------
　　C/C++ 在 2011 年发表了新的标准 C11/C++11，开始出现带边界检查的函数接口，定义了新的安全函数。这些函数与原本对应的函数含义相同，但用法不同。新语法在不同的编译器有着不同的反应。
<ol>
　　<li>较新版本 VS 默认检查函数的安全性，一些函数在不加安全标签 _s 的情况下会强制报错。</li>
　　<li>对于 DEVC++，当源代码为 .c 格式时，两种类型的语法均可通过编译；而 .cpp 格式则不能用新语法编译。</li>
</ol>

　　因此，当使用 DEVC++ 编译时，最好使用老版本语法；用 VS 编写时要考虑到跨平台代码兼容。<br />（VS 安全检查可以关闭，依次选中项目——属性——C/C++——代码生成———禁用安全检查。）<br />
